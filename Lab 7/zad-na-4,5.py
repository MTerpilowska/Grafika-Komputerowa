...
def compile_shaders():
    vertex_shader_source = """
        ...
        void main(void) {
            int next_OY = int(gl_InstanceID/10);

            gl_Position = P_matrix * V_matrix * M_matrix * (position + (gl_InstanceID % 10 * vec4(1, 0, 0, 0)) + (next_OY * vec4(0, 1, 0, 0)));
            vertex_color = diff_vertex_colors;
        }
    """
...
def startup():
    ...
    # nowa tablica kolorów (na górze) i bufor (na dole)
